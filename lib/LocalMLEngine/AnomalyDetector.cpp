...
void AnomalyDetector::calculateStats(float *values, uint8_t count, SensorStats &stats)
{
    ...
    // Calculate mean
    float sum = 0;
    stats.minVal = values[0];
    stats.maxVal = values[0];

    for (int i = 0; i < count; i++)
    {
        sum += values[i];
        if (values[i] < stats.minVal)
            stats.minVal = values[i];
        if (values[i] > stats.maxVal)
            stats.maxVal = values[i];
    }
...
void AnomalyDetector::printStatistics() const
{
    ...
    Serial.print("  Min: ");
    Serial.println(moistureStats.minVal);
    Serial.print("  Max: ");
    Serial.println(moistureStats.maxVal);
    ...
    Serial.print("  Min: ");
    Serial.println(temperatureStats.minVal);
    Serial.print("  Max: ");
    Serial.println(temperatureStats.maxVal);
    ...
    Serial.print("  Min: ");
    Serial.println(humidityStats.minVal);
    Serial.print("  Max: ");
    Serial.println(humidityStats.maxVal);
    ...
    Serial.print("  Min: ");
    Serial.println(lightStats.minVal);
    Serial.print("  Max: ");
    Serial.println(lightStats.maxVal);
