...
struct SensorData {
    ...
    // Constructor
    SensorData() : moisture(0), temperature(0), humidity(0), lightLevel(0) {}
    SensorData(float m, float t, float h, float l) : moisture(m), temperature(t), humidity(h), lightLevel(l) {}
};
...
struct SensorStats {
    ...
    // Constructor
    SensorStats() : mean(0), variance(0), stdDev(0), min(999999), max(-999999), sampleCount(0) {}
};
