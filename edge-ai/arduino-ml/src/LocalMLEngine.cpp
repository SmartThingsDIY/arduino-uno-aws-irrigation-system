...
bool LocalMLEngine::detectAnomaly(const SensorData& data) {
    ...
    const float ANOMALY_THRESHOLD = 0.997; // 99.7% confidence
    
    bool isAnomaly = anomalyScore > ANOMALY_THRESHOLD;
...
Action LocalMLEngine::getImmediateAction(int sensorIndex, const SensorData& data) {
    ...
    if (detectAnomaly(modifiedData)) {
        if (failsafeEnabled) {
            ...
            if (modifiedData.moisture > threshold * 1.2) { // 20% above threshold
...
WaterAmount LocalMLEngine::mapToWaterAmount(float prediction) {
    ...
    if (prediction > 0.75) return HIGH_WATER;
    if (prediction > 0.5) return MEDIUM_WATER;
    if (prediction > 0.25) return LOW_WATER;
    return NO_WATER;
}
...
unsigned int LocalMLEngine::calculateWaterDuration(WaterAmount amount) {
    ...
    switch (amount) {
        case LOW_WATER:    return 500;   // 0.5 seconds = 50ml
        case MEDIUM_WATER: return 1000;  // 1 second = 100ml
        case HIGH_WATER:   return 2000;  // 2 seconds = 200ml
        default:           return 0;
    }
}
