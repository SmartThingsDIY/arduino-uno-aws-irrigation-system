...
bool AnomalyDetector::isSensorFault(const SensorData& data) {
    ...
    if (isSensorDisconnected(data)) return true;
    
    // Check for out-of-range values
    if (isSensorOutOfRange(data)) return true;
...
bool AnomalyDetector::isSensorDisconnected(const SensorData& data) {
    ...
    if (data.moisture <= 5 || data.moisture >= 1018) return true;
    
    // Temperature sensor disconnected (usually reads extreme values)
    if (data.temperature <= -50 || data.temperature >= 80) return true;
    
    // Humidity sensor disconnected (usually reads 0 or 100)
    if (data.humidity <= 1 || data.humidity >= 99) return true;
    
    // Light sensor disconnected (usually reads 0 or 1023)
    if (data.lightLevel <= 5 || data.lightLevel >= 1018) return true;
...
bool AnomalyDetector::isSensorOutOfRange(const SensorData& data) {
    ...
    if (data.moisture < 0 || data.moisture > 1023) return true;
    
    // Temperature: -40 to 70°C (typical sensor range)
    if (data.temperature < -40 || data.temperature > 70) return true;
    
    // Humidity: 0-100%
    if (data.humidity < 0 || data.humidity > 100) return true;
    
    // Light: 0-1023 (analog reading)
    if (data.lightLevel < 0 || data.lightLevel > 1023) return true;
...
float AnomalyDetector::calculateZScore(float value, const SensorStats& stats) {
    if (stats.stdDev == 0) return 0; // Avoid division by zero
